...
import { sp, Web, PermissionKind, RegionalSettings } from "@pnp/sp/presets/all";
import { graph, } from "@pnp/graph";
import * as $ from 'jquery';
import { IEventData } from './IEventData';
import { IEventAgenda } from './IEventAgenda';
import { IAgenda } from './IAgenda';
import * as moment from 'moment';
import { SiteUser } from "@pnp/sp/presets/all";
//import { SiteUser } from "../model/SiteUsers";
...
export default class spservices {
  ...
  public async getChoiceFieldOptions(siteUrl: string, listId: string, fieldInternalName: string): Promise<{ key: string, text: string }[]> {
    ...
    try {
      const web = Web(siteUrl);
      const results : IChoiceFieldInfo= await web.lists.getById(listId)
        .fields
 ...
 /**
   ...
   * @memberof spservices
   */
  public async getEventsAgenda(siteUrl: string, listId: string, eventStartDate: Date, eventEndDate: Date): Promise<IEventAgenda[]> {

    let events: IEventAgenda[] = [];
    if (!siteUrl) {
      return [];
    }
    try {
...
console.log("SP Service call");
  ...
  public async getSiteRegionalSettingsTimeZone(siteUrl: string) {
    let regionalSettings: typeof RegionalSettings;
    try {
      const web = Web(siteUrl);
